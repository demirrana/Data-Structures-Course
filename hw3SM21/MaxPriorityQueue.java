...
public class MaxPriorityQueue<K,V> extends AbstractMaxPriorityQueue<K,V> {
    ...
    public MaxPriorityQueue(Comparator<K> comp) { 
    ...
    public void downheap(Position<Entry<K,V>> entryPos) {
        ...
        Position<Entry<K,V>> walk = entryPos; //Verilen position ile baslar
         ...
         while (heap.left(walk) != null) { //Sol node'u olmayan bir node'a gelene kadar
            ...
            Position<Entry<K,V>> bigPos = leftPos; //Sag ve sol node'lardan en kucuk elemani iceren position
            ...
            if (heap.right(walk) != null) { //Sag node da varsa en kucuk elemani bulmak icin sol node ile karsilastirir
                ...
                if (compare(rightPos.getElement() , bigPos.getElement()) > 0)
                    bigPos = rightPos;
            }

            if (compare(bigPos.getElement() , walk.getElement()) <= 0) //Child'daki eleman parent'indan buyuk ya da ona esitse yapacak bir sey kalmaz
                break;

            //Child daha kucuk oldugunda ikisini swap eder
            Entry<K,V> entry = bigPos.getElement();
            heap.set(bigPos , walk.getElement());
            heap.set(walk , entry);

            walk = bigPos; //Iterasyon her seferinde bir alt seviyeye (child'a) iner
        }
    }
    ...
    public Entry<K,V> removeMax() {
        ...
        else if (heap.size() == 2) { //Root ve left child'i varsa left child'i siler
        ...
        else if (heap.size() == 3) { //Root ile child'lari varsa right child'ini siler
